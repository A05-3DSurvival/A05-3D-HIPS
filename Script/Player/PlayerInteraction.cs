...
public class PlayerInteraction : MonoBehaviour
{
        ...
        {            
            ...
            if (Physics.Raycast(ray, out hit, maxCheckDistance, layerMask))
            {
                if (hit.collider.gameObject != lastInteractObject)
                {
                    ...
                    structureObject = hit.collider.GetComponent<StructureObject>(); // structureObject 초기화
                    SetDescriptionText();
                }
            }
            else
    ...
    {
        if (context.phase == InputActionPhase.Started && structureObject != null)
        {
            if (structureObject != null)
            {
                UIInventory.Instance.RepairDoor(structureObject);
            }
        }
    }
}
